...
class Checkin extends Api {
    ...
    function create($query, $data ) {
        ...
        if ( $data["limit"] === "hourly") {  // 按小时
            $rows = $ci->search(["user_id"=>$user_id, "time_after"=>date('Y-m-d H:i:s', $time-3600)]);
            if ( $rows["total"] > 0 ) {
                throw new Excp("已经签过了, 一小时后再试.", 403, ["rows"=>$rows, "user_id"=>$user_id]);
            }

        } elseif ( $data["limit"] === "daily") { // 按天
            $rows = $ci->search(["user_id"=>$user_id, "time_after"=>date("Y-m-d 00:00:00")]);
            if ( $rows["total"] > 0 ) {
                throw new Excp("今天已经签过了, 明天再试.", 403, ["rows"=>$rows, "user_id"=>$user_id]);
            }

        } elseif ( $data["limit"] === "weekly") { // 按周
            $rows = $ci->search(["user_id"=>$user_id, "time_after"=>date('Y-m-d 00:00:00', strtotime('monday this week'))]);
            if ( $rows["total"] > 0 ) {
